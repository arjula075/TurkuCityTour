...
import GameUserAssignment from './GameUserAssignment';


export default function GameAdminTab() {
    ...
    useEffect(() => {
        async function loadLocations() {
            if (!selectedGameId) {
                // No game selected yet, skip loading
                return;
            }
            try {
                const locs = await adminLocations.fetchByGame(selectedGameId);
                setLocations(locs);
    ...
    useEffect(() => {
        if (selectedGameId) {
            adminLocations.fetchByGame(selectedGameId).then(setLocations);
        }
    }, [selectedGameId]);
    ...
    const handleAddLocation = async () => {
        ...
        const newLoc = {
            ...
            display_order: locations.length + 1, // optional but useful
        };

        const created = await adminLocations.insert(newLoc);
